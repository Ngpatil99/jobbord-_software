...
const LibraryCard = (props) => {
  ...
  const isCardSelectedArray = props?.selectMutipleQuestion
  ...
  const onClickTryQuestion = (e) => {
    e.stopPropagation();
    let url = `https://www.assessment.theeliteqa.com/test4?id=${
      props.data?._id
    }&page=${props?.sourceSelected ? "library" : "mcqbulk"}`;
    window.open(url, "_blank");
  };

  const removeFromTest = async (e) => {
    e.stopPropagation();
    const filterDeletedData = createTestContext.question.filter(
      (data) => data.questions._id !== props.data._id
    );
    createTestContext.addDeleteQuestionData(filterDeletedData);
    //let searchSkill = props?.data?.skillsId?.map((data) => { return data?.skills?.skills })
    createTestContext.questionOverview.forEach((data) => {
      if (
        createTestContext.question
          .filter(
            (questionData) => questionData.questions._id === props.data._id
          )
          .map((questionData) => questionData.skillId)[0] ===
          createTestContext.addedSkills
            .filter((data1) => data1.skill === data.skillName)
            .map((data1) => data1.skillId)[0] &&
        data.totalNoOfQuestion > 0
      ) {
        const filterEasyDeletedData = data.easyQuestion.question.filter(
          (data) => data._id !== props?.data?.id
        );
    ...
    const res = await axios.put(
      ...
      {
        ...props.data,
        noOfTimesUsed: props.data.noOfTimesUsed,
      },
      { headers: { token: token } }
    );

    toast.success("question is removed from test!");
  };
  ...
  const getType = (type, skill) => {
    createTestContext.addedSkills.forEach((data) => {
      if (data.skill === skill && type === data.questionType) {
        return false;
      } else {
        return true;
      }
  ...
  const addSingleToLibrary = async () => {
    let searchSkill = props?.data?.skillsId?.map((data) => {
    ...
    const res = await axios.put(
      `${backend_url}question/updateQuestion/${props.data._id}`,
      {
        ...props.data,
        noOfTimesUsed:
          props.data.noOfTimesUsed > 0 ? props.data.noOfTimesUsed - 1 : 0,
      },
      { headers: { token: token } }
    );
    ...
    createTestContext.addedSkills.forEach((skillData) => {
      if (
        ...
        skillData.questionType === props.data.type
      ) {
        const level = skillData?.level.split(",");
        ...
        if (difficultyLevelId?.level === "easy") {
          easyQuestion = easyQuestion + parseInt(1);
        }
        if (difficultyLevelId?.level === "medium") {
          mediumQuestion = mediumQuestion + parseInt(1);
        }
        if (difficultyLevelId?.level === "hard") {
    ...
    createTestContext.questionOverview.forEach((data) => {
      if (
        ...
        if (difficultyLevelId?.level === "easy") {
          data.totalNoOfQuestion = data.totalNoOfQuestion + 1;
          data.totalScore = data.totalScore + props.data.score;
          data.easyQuestion.question.push(props?.data);
        }

        if (difficultyLevelId?.level === "medium") {
          data.totalNoOfQuestion = data.totalNoOfQuestion + 1;
          data.totalScore = data.totalScore + props.data.score;
          data.mediumQuestion.question.push(props?.data);
        }

        if (difficultyLevelId?.level === "hard") {
          data.totalNoOfQuestion = data.totalNoOfQuestion + 1;
          data.totalScore = data.totalScore + props.data.score;
          data.hardQuestion.question.push(props?.data);
        }
      }
    });
  ...
  const addToLibrary = async (e) => {
    e.stopPropagation();
    if (props?.data?.skillsId.length > 1) {
      props.onAddToLibrary(props.data);
    } else {
      if (
        !checkSearchedSkillAddedOrNot(props?.data?.skillsId, props.data.type)
      ) {
        await createSkillObject(
          props?.data?.skillsId[0].skills?.skills,
          props?.data.type,
          props?.data?.skillsId[0].skills?._id
        );
        ...
        createTestContext.questionOverview.forEach((data) => {
          if (
            searchSkill.includes(data.skillName) &&
            createTestContext.addedSkills
              .filter((data1) => data1.skill === searchSkill[0])
    ...
    //    if( !checkSearchedSkillAddedOrNot(props?.data?.skillsId,props.data.type)){

    //     if(props?.data?.skillsId.length>1){
    //     props.onAddToLibrary(props.data)
    //     }else{
    //         await createSkillObject(props?.data?.skillsId[0].skills?.skills, props?.data.type,props?.data?.skillsId[0].skills?._id)
    //         await new Promise(resolve => setTimeout(resolve, 100));
    //         setisSkillAdded(true)
    //     }
    //    }else{
    //     createTestContext.setQuestions(props.data)
    //     let searchSkill = props?.data?.skillsId?.map((data) => { return data?.skills?.skills })
    //     //addMissingSkills(searchSkill,createTestContext.addedSkills)
    //     createTestContext.addedSkills.forEach((skillData) => {
    //         if (searchSkill.includes(skillData.skill) && skillData.questionType === props.data.type) {
    //             const level = skillData?.level.split(",")
    //             let easyQuestion = parseInt(level[0]?.replace(/\D/g, ""))
    //             let mediumQuestion = parseInt(level[1]?.replace(/\D/g, ""))
    //             let hardQuestion = parseInt(level[2]?.replace(/\D/g, ""))
    ...
    //     createTestContext.questionOverview.forEach((data) => {
    //         if (searchSkill.includes(data.skillName) && data.totalNoOfQuestion > 0) {
    //             if (difficultyLevelId?.level === "easy") {
    //                 data.totalNoOfQuestion = data.totalNoOfQuestion + 1
    //                 data.totalScore = data.totalScore + props.data.score
    //                 data.easyQuestion.question.push(props?.data)
    ...
    //         }
    //     })
    //     createTestContext.setFilterQuestionOverview(createTestContext.questionOverview)
    //     toast.success('Questions is added into test')
    //    }
  };

  const truncateQuestionHeader = (questionHeader) => {
    if (questionHeader?.length > 73) {
      return questionHeader.substring(0, 73) + "...";
    }
  ...
  return (
    <div
      ...
      className="choose-library-card-container"
      style={
        cardSelected
          ? { border: " 1px solid #00C49A", cursor: "pointer" }
          : { cursor: "pointer" }
      }
    >
      <div className="choose-library-header-container">
        <div className="choose-header">
          {props.isQuestionAdded ? (
            <></>
          ) : (
            <>
              {isCardSelectedArray.includes(props.data._id) ? (
                <svg
                  onClick={(e) => {
                    e.stopPropagation();
                    props.onQuestionDeselect(props.data);
                  }}
                ...
                <svg
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    props.onQuestionClickSelect(props.data);
                  }}
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
          ...
          <label>
            {props?.name === "mcqbulkpreview"
              ? `Q${props?.questionNo + 1}.`
              : ""}{" "}
            {truncateQuestionHeader(Section_header)}
          </label>
        </div>
        {props.isQuestionAdded ? (
          <div onClick={(e) => removeFromTest(e)}>
            <span className="added">
              ...
              >
                <path
                  d="M20.3504 8.44518C20.3132 8.28877 20.1564 8.19216 19.9994 8.22937C19.8429 8.26677 19.7463 8.42378 19.7837 8.58018C19.9369 9.22262 20.0148 9.90105 20.0148 10.5969C20.0148 15.5671 15.8805 19.6107 10.7987 19.6107C5.71684 19.6107 1.58263 15.5671 1.58263 10.5969C1.58263 5.62624 5.71684 1.58243 10.7987 1.58243C12.8226 1.58243 14.7559 2.22286 16.3894 3.43453C16.5182 3.53033 16.701 3.50333 16.7968 3.37412C16.8926 3.24492 16.8656 3.06251 16.7364 2.9667C15.0019 1.67983 12.9486 1 10.7987 1C5.39562 1 1 5.30502 1 10.5969C1 15.8883 5.39562 20.1934 10.7987 20.1934C16.2018 20.1934 20.5974 15.8883 20.5974 10.5969C20.5974 9.85585 20.5142 9.13201 20.3504 8.44518Z"
                  fill="#00C49A"
                  stroke="#00C49A"
                  stroke-width="1.5"
                />
                <path
                  d="M10.5941 14.9337C10.5169 14.9337 10.4429 14.9031 10.3881 14.8485L4.9252 9.38642C4.81139 9.27262 4.81139 9.08821 4.9252 8.9744C5.0388 8.8606 5.22321 8.8606 5.33702 8.9744L10.5819 14.2185L20.49 3.12071C20.5972 3.0007 20.7812 2.9903 20.9012 3.09751C21.0212 3.20471 21.0316 3.38872 20.9246 3.50873L10.8113 14.8365C10.7579 14.8963 10.6823 14.9315 10.6023 14.9337C10.5995 14.9337 10.5969 14.9337 10.5941 14.9337Z"
                  fill="#00C49A"
                  stroke="#00C49A"
                  stroke-width="1.5"
                />
            ...
            <span className="remove-from-test">- Remove From Test</span>
          </div>
        ) : (
          <>
            {isCardSelectedArray.includes(props.data._id) ? (
      ...
      {props.sourceSelected !== "Draft" ? (
        <div className="choose-library-bottom-container">
          ...
          <div className="library-right-side">
            ...
            {/* {props.userClientID === "632c16db596546cfa858136f" || props.sourceSelected !== "EliteQA Library" ?
                            <span data-tip="Delete question" onClick={onClickDeleteButton} style={{ cursor: 'pointer' }} >
                                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1.875 3.75H3.125H13.125" stroke="#827C7C" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M11.875 3.75V12.5C11.875 12.8315 11.7433 13.1495 11.5089 13.3839C11.2745 13.6183 10.9565 13.75 10.625 13.75H4.375C4.04348 13.75 3.72554 13.6183 3.49112 13.3839C3.2567 13.1495 3.125 12.8315 3.125 12.5V3.75M5 3.75V2.5C5 2.16848 5.1317 1.85054 5.36612 1.61612C5.60054 1.3817 5.91848 1.25 6.25 1.25H8.75C9.08152 1.25 9.39946 1.3817 9.63388 1.61612C9.8683 1.85054 10 2.16848 10 2.5V3.75" stroke="#827C7C" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M6.25 6.875V10.625" stroke="#827C7C" stroke-linecap="round" stroke-linejoin="round" />
                                    <path d="M8.75 6.875V10.625" stroke="#827C7C" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
          ...
          <div className="draft-library-left-side">
            ...
            {/* {props.userClientID === "632c16db596546cfa858136f" || props.sourceSelected !== "EliteQA Library" ?
                            <span onClick={onClickDeleteButton} style={{ cursor: 'pointer' }} >
                                ...
                                <p style={{ color: '#827C7C' }} >Delete Question</p>
                            </span> : <></>
                        } */}
          </div>
