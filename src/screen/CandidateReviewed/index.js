...
function CandidateReviewed() {
  ...
  const handleCheckBox = (length) => {
    if (length !== undefined) {
      setSelectedAll(true);
      setSelectedCandidates(context.reviewedCandidates);
    }
  };
  ...
  const updateCandidateStatusInBulk = async (candidateId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        const filterShortlistedCandidate = context.reviewedCandidates.filter(
          (data) =>
            selectedCandidates.some(
              (candidate) => candidate.invite._id !== data.invite._id
            )
        );
        context.handleReviewedCandidates(filterShortlistedCandidate);
      }
    } catch (error) {
      console.log(error);
  ...
  const moveToPassed = (candidateId) => {
    try {
      const candidateData = context.reviewedCandidates.filter(
        (data) => data.invite._id === candidateId
      );
      const statusIndex = candidateData[0].invite.status.findIndex(
        (status) => status.currentStatus === "reviewed"
      );
      if (statusIndex !== -1) {
          ...
          <div className="test-taken-content">
              ...
              >
                ...
                >
                  <rect
                    ...
                    fill="#FF6812"
                  />
                  <g clip-path="url(#clip0_3088_2045)">
                    <path
            ...
            <div className="candidate-test-heading">
              <div className="test-heading-left">
                ...
                <span>
                  Reviewed Candidates ({context.reviewedCandidates.length})
                </span>
                <div>
                      ...
                      {searchData.map((data, index) => {
                        return (
                          <span
                            key={index}
                            onClick={() => {
                              context.handleReviewedCandidates([data]);
              ...
              <div className="test-heading-right">
                <select
                  onChange={(e) => {
                    context.filterCandidates("reviewed", e.target.value);
                  }}
                >
            ...
            {context.reviewedCandidates.length === 0 &&
            context.loading === false ? (
              <span className="msg">No candidates reviewed.</span>
            ) : (
              <div className="candidate-card-container">
                {context.reviewedCandidates
                  .sort(
                    (a, b) =>
                      ...
                      <TestTakenCard
                        ...
                        finishedDate={
                          data?.invite.status.filter(
                            (data) => data.currentStatus === "reviewed"
                          )[0]?.statusDate
                        }
                        status={
                          data.invite.status[data.invite.status.length - 1]
...
export default CandidateReviewed;
