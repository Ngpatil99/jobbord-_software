...
import TestShortlistedCard from "./TestShortlistedCard";
...
function CandidateShortlisted() {
  ...
  const moveToPassed = (candidateId) => {
    try {
      const candidateData = context.shortlistedCandidates.filter(
        (data) => data.invite._id === candidateId
      );
      const statusIndex = candidateData[0].invite.status.findIndex(
        (status) => status.currentStatus === "shortlisted"
      );
      if (statusIndex !== -1) {
        candidateData[0].invite.status.splice(statusIndex, 1);
      }
      ...
      const filterShortlistedCandidate = context.shortlistedCandidates.filter(
        (data) => data.invite._id !== candidateId
      );
      context.handleShortlistedCandidates(filterShortlistedCandidate);
      setrefresh({});
    } catch (error) {
      console.log(error);
      toast.error(error);
          ...
          <div className="test-taken-content">
              ...
              >
                ...
                >
                  <rect
                    ...
                    fill="#615D5B"
                  />
                  <g clip-path="url(#clip0_3111_2011)">
                    <path
            ...
            <div className="candidate-test-heading">
              <div className="test-heading-left">
                <span>
                  Shortlisted Candidates ({context.shortlistedCandidates.length}
                  )
                </span>
                <div>
                  ...
                  {searchData.length === 0 ? null : (
                    <div className="search-container-shortlisted">
                      {searchData.map((data, index) => {
                        return (
                          <span
                            key={index}
                            onClick={() => {
                              context.handleShortlistedCandidates([data]);
              ...
              <div className="test-heading-right">
                <select
                  onChange={(e) => {
                    context.filterCandidates("shortlisted", e.target.value);
                  }}
                >
            ...
            {context.shortlistedCandidates.length === 0 &&
            context.loading === false ? (
              <span className="msg">No candidates shortlisted.</span>
            ) : (
              <div className="candidate-card-container">
                {context.shortlistedCandidates
                  .sort(
                    (a, b) =>
                  ...
                  .map((data, index) => {
                    return (
                      <TestShortlistedCard
                        moveToPass={(id) => moveToPassed(id)}
                        finishedDate={
                          data?.invite.status.filter(
                            (data) => data.currentStatus === "shortlisted"
                          )[0]?.statusDate
                        }
                        status={
            ...
            {context.loading
              ? loaderArray.map((data) => {
                  return <CandidateCardSkeleton page="shortlisted" />;
                })
              : null}
          </div>
        </div>
