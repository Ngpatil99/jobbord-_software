...
function CandidateResult() {
  ...
  const [candidateRank, setCandidateRank] = useState(0);
  ...
  const getCandidateById = async () => {
    try {
      const response = await axios.get(
        `${backend_url}getCandidateResultByInviteId/${candidateId}`
      );
      setTest(response.data.data.test_id);
      findRank(response.data.data.test_id);
      ...
      const testsummaryData = await axios.get(
        `${backend_url}getTestSummary/${response.data.data.test_id._id}`
      );
  ...
  const findRank = async (id) => {
    console.log(id._id);
    const token = getCookie("Xh7ERL0G");
    const response = await axios.get(
      `${backend_url}invites/getInvite/${id._id}?status=${`appeared`}`,
      { headers: { token: token } }
    );
    ...
    response.data.data
      ...
      .map((data, index) => {
        if (data.invite._id === candidateId) {
          setCandidateRank(index + 1);
        }
      });
  };
