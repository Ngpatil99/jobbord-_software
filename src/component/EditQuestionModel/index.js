...
import "../AddQuestionPopup/index.css";
...
const EditQuestionModel = (props) => {
  ...
  const getQuestionById = async () => {
    try {
      ...
      const response = await axios.get(
        `${backend_url}question/findQuestion/${props?.data._id}`,
        { headers: { token: token } }
      );
      setanswerObj(response.data.data.answersObjectArray);
      setsourceSelected(
        props.data?.clientId?._id === "632c16db596546cfa858136f"
          ? "EliteQA Library"
          : props.data?.clientId?._id === decode?.client?._id &&
      ...
      setRichText(
        response.data.data.isEditorEnable === undefined
          ? false
          : response.data.data.isEditorEnable
      );
      ...
      // response.data.data.answersObjectArray.map((data, index) => {
      //     if (index === 0) {
      //         setAnsOneContent(data.html)
      //     } else if (index === 1) {
      //         setAnsTwoContent(data.html)
      //     } else if (index === 2) {
      //         setAnsThreeContent(data.html)
      //     } else if (index === 3) {
      //         setAnsFourContent(data.html)
      //     } else if (index === 4) {
      //         setAnsFiveContent(data.html)
      //     } else if (index === 5) {
      //         setAnsSixContent(data.html)
      //     }
      // })
    } catch (error) {
      console.log(error);
  ...
  const validate = (question) => {
    ...
    const regex = /^\s*$/;
    if (question.Section_header === "" || regex.test(question.Section_header)) {
      return false;
    }
    if (question.answersObjectArray.length === 0) {
      return false;
    }
    if (question.correctAnswerObjectArray.length === 0) {
      return false;
    }
    if (question.correctAnswerType === "") {
      return false;
    }

    if (question.question === "" || regex.test(question.question)) {
      return false;
    }
    if (question.difficultyLevelId === "") {
      return false;
    }
    if (question.score === 0) {
      return false;
    }
    if (question.skillsId.length === 0) {
      return false;
    }
    if (question.topicId.length === 0) {
      return false;
    }
    return true;
  };

  const moveToLibrary = async (question) => {
    const token = getCookie("Xh7ERL0G");
    // const decode = jwtDecode(token);
    ...
    if (res.status === 200) {
      toast.success("Question moved to library successfully");
      props.refreshData("draft");
    }
  };
  const saveQuestion = async (status) => {
    ...
    try {
      ...
      if (validation(status)) {
        ...
        props.onClickSave(res.data.data);
        setQuestionTitle("");
        setDifficulty("");
        setskills([]);
        settopics([]);
        ...
        props.refreshData(props?.data?.status);
      }
    } catch (error) {
      setLoading(false);
    }
  ...
  const removeAnwserObject = (optionId) => {
    if (answerObj.length === 2) {
      return toast.error("Atleast two options are mandatory.");
    } else {
      const newAnwserObject = answerObj.filter(
        (data) => data.optionId !== optionId
      );
      const newCorrectAnsObject = correctObj.filter(
        (data) => data !== optionId
      );
  ...
  const isPositiveNumber = (num) => {
    ...
    return parsedNum >= 0 && /^[0-9]+$/.test(num);
  };
  ...
  const handleQuestionModel = () => {
    //set not save true only if question data get changed
    if (
      props?.data?.Section_header !== questionTitle ||
      props?.data?.score !== score ||
      props?.data?.difficultyLevelId._id !== difficulty ||
      props?.data?.correctAnswerType !== correctAnswerType ||
      props?.data?.html !== editorContent ||
      props?.data?.isEditorEnable !== richText
    ) {
      setNotSave(true);
    } else {
      props?.onClickCancel();
      ...
      <div className="add-question-model-container">
        <div className="header-and-button-container">
          ...
          <div className="button-container">
            ...
            <button
              style={{ background: "#00C49A", width: "150px" }}
              onClick={() => {
                props.data.status === "draft"
                  ? saveQuestion("draft")
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <label>
                    {props.data.status === "draft" ? "Save As Draft" : "Save"}
                  </label>
                </>
              )}
            </button>

            {validate(props.data) && props.data.status === "draft" && (
              <button
                style={{ background: "#FF6812", color: "#fff" }}
                onClick={(e) => {
                  e.stopPropagation();
                  moveToLibrary(props.data);
                }}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="question-container">
          <div className="left-container">
            <div className="add-question-title-container">
              ...
              >
                {countWords() > 8
                  ? "Question title should not exceed 8 word."
                  : null}
              </label>
            </div>
                      ...
                      >
                        ...
                        <div className="editor">
                          <SunEditor
                            ...
                            setContents={
                              answerObj[index].html !== undefined
                                ? answerObj[index].html
                                : answerObj[index].option + ""
                            }
                            onChange={(content) => {
